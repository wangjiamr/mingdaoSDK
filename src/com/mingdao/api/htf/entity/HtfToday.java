 ...
 */
public class HtfToday implements Serializable {
    ...
    public List<HtfTradeItem> getItemList(){
        JSONArray  todayItems=getItem();
        List<HtfTradeItem> itemList=null;
        if(todayItems!=null&&!todayItems.isEmpty()){
            itemList=new ArrayList<HtfTradeItem>();
            for(Object item:todayItems){
                HtfTradeItem todayItem= (HtfTradeItem) JSONObject.toBean((JSONObject) item, HtfTradeItem.class);
